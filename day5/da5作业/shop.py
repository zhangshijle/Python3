...
shop_shangpin={"手机":4598,"彩电":1580,"沙发":2250,"汽车":125000,"水杯":65,"衬衣":128,"Mac":12898}  #定义商品


shangpin = list(shop_shangpin.keys())  #根据key取出商品的名称
jiage = list(shop_shangpin.values())   #根据value 取出商品的价格
shangpin_num = range(len(shangpin)) #商品队列长度,即有多少商品
shangpin_num_c = len(shangpin)
list1 = []  #已经购买的商品价格列表
list2 = []  #已经购买的商品名称列表
...
def shangpin_select_func(): #让用户选择商品的函数
    num = 0
    print("\033[31;1m商品列表\033[1m".center(100,"#"))
    print("\033[31;1m商品编号 商品名称 商品价格\033[0m")
    for shangpin_list in shangpin:
        SHANGPIN_NUM = num
        print("%s       %s       %s" % (SHANGPIN_NUM,shangpin_list,shop_shangpin[shangpin_list])) #自动匹配商品列表长度,并给商品加上商品编号
        num += 1
...
def shop_input_func(): #获取用户选择商品的函数
    ...
    gongzi = input("输入你准备花多少钱购物:") #自定义欲消费的金额,也可提前设置好
    flag = gongzi.isdigit()
    if flag == True:
        global  gongzi
        gongzi = int(gongzi)
        shop_select_shangpin()
    else:
        print("工资输入错误,请输入不包含非法的字符的阿拉伯整数:")
        shop_input_func()


def shop_select_shangpin():
    global  gongzi
    flag = True #标记位
    while flag:
        print("\033[31;1m商品选购\033[1m".center(100,"-"))
        #print("当前购物商城的商品有%s" % shangpin)
        select  = input("请输入你要选购的商品编号即可购买,并且小于%s的数字(如1或2):" % shangpin_num_c)  #默认获取到的是字串,要用int转换成数值,否则根据输入的数字取下标元素报错,"TypeError: list indices must be integers or slices, not str"
        ...
        if flag == True:
            ...
            QUIT_NUM = input("请问您继续购物还是要退出结算? 输入Q/q结算退出,输入Y/y继续,输入C/c查询已加入到购物车到商品,输入E/e编辑购物车,输入V/v查看历史购物记录:") #让用户选择是继续,退出还说查询购买的商品或退出
            ...
            if quit_num == "q": #如果输入q
                ...
                while True:
                    C_QUIT_NUM = input("请问您继续购物还是要退出结算? 输入Q/q结算退出,输入Y/y继续购物,输入C/c查询已加入到购物车到商品,输入E/e编辑购物车,输入V/v查看历史购物记录:")
...
def user_edit_gouwuche():  #编辑购物车函数
        ...
        user_edit_Fangfa = input("请选择是要添加商品还是删除商品,A/a添加,D/d为删除,C/c退出结算,R/r返回上一级选购商品!:") #选择添加商品还是删除商品
        user_edit_fangfa = user_edit_Fangfa.lower()
        while True:
            print(user_edit_fangfa)
            if  user_edit_fangfa == "r":
                shop_select_shangpin()
                break

            if user_edit_fangfa == "d":
                print("\033[31;1m删除购物\033[1m".center(100,"-"))
                user_edit_del_select  = input("请输入你要删除的商品名称(如 沙发):")  #默认是字串,要用int转换成数值,否则根据输入的数字取下标元素报错,"TypeError: list indices must be integers or slices, not str"
                #name = "%s" % (shangpin[select]) #以用户输入的数字为下标,取出该下标的物品名称
                ...
                if user_edit_del_select in list2:
                    ...
                    print("输入错误,请重新输入要删除的商品名称(如 衬衣)")
                    print("您当前的购物车有%s" % list2)
                    continue

            if  user_edit_fangfa == "a": #假如用户输入的是a
                print("\033[31;1m添加购物\033[1m".center(100,"-"))
                user_edit_add_select  = input("请输入你要添加的商品名称,此名称必须是存在商品列表当中并切您未曾购买过的商品(如 衬衣):") #获取用户输入
                if  user_edit_add_select in shangpin: #假如用户输入的存在于商品名称列表
                    if  user_edit_add_select in list2: #假如商品存在列表后有存在于已购买商品列表
                        ...
                        list1.append(shop_shangpin[user_edit_add_select]) #更新价格列表
                        user_edit_dict = dict(zip(list2,list1)) #更新已购买字典列表
                        #print(list2)
                        #print(list1,"list1")
                        #print(shop_shangpin[user_edit_add_select])
                        import  time
            ...
            if  user_edit_fangfa == "c": #假如用户输入的是c要退出结算
                ...
                print("输入错误,请输入A/a添加商品或D/d删除商品,R/r返回上一级")
                print("您的购物车商品为%s " % list2)
                print("当前商城商品有%s" % shangpin)
                user_edit_gouwuche()
                break
...
def user_logout_func():  #用户退出函数
    ...
    with open("history.txt","a+") as f: #以追加读写的方式打开用户购物记录文件,可用保存a+方式可以保留之前的购物记录,w会晴空之前的记录
        for i in list2: #循环用户购买到的商品
            f.write("您在%s购买的商品有-->%s\n" % (user_logout_now.strftime("%Y年%m月%d日%H点%M分%S"),i))
            f.flush() #立即将内存的数据保存在硬盘
    print("\033[31;0m您在%s 购物一共花费%d元,够买到了%s,欢迎下次再来购物!\033[0m" % (user_logout_now.strftime("%Y:%m:%d %H:%M:%S"),logout_num,list2)) #输出用户本次的购物清单


def user_address():
    print("\033[31;1m收获地址\033[1m".center(100,"-"))
...
def user_baitiao(): #用户白条函数
    print("\033[31;1m白条界面\033[1m".center(100,"-"))
    Command = input("您的金额不足,是否需要使用白条额度?,Y立即使用,N继续购物") #让用户选择是否要使用白条
    command = Command.lower()
    global y_user_baitiao
    if command == "y":
        global gongzi
        global user_baitiao_y #全局变量
        baitiao_user = input("请输入白条账户名:")
        baitiao_password = input("请输入用户支付密码:")
        if baitiao_user == "zhangshijie" and baitiao_password == "123456":
