function bigUpload () {
	...
	this.processFiles = function() {
		...
		this.$(this.settings.submitButton).value = '暂停上传';
		...
		if(fileSize > this.settings.maxFileSize) {
			this.printResponse('您选定的文件太大了.', true);
			return;
		}
	...
	this.sendFile = function (chunk) {
		...
		if(this.uploadData.paused === true) {
			...
			this.printResponse('已暂停上传.', false);
			return;
		}
	...
	this.sendFileData = function() {
		var data = 'key=' + this.uploadData.key + '&name=' + this.uploadData.file.name+ parent.settings.scriptPathParams;
		xhr = new XMLHttpRequest();
		...
		xhr.onreadystatechange = function() {
				if(xhr.readyState == 4) {
					...
					parent.printResponse('文件已成功上传.', false);

					parent.success(response);

                    window.location.href=parent.settings.redirectUrl;
				}
			};
	...
	this.abortFileUpload = function() {
		...
		xhr.onreadystatechange = function() {
				if(xhr.readyState == 4) {
					...
					parent.printResponse('已取消上传.', true);
				}
	...
	this.pauseUpload = function() {
		...
		this.$(this.settings.submitButton).value = '继续上传';
	};
	...
	this.progressUpdate = function(progress) {
		...
		if(progress % 5 === 0) {
			...
			this.$(this.settings.timeRemainingField).textContent = '还剩 ' + timeLeft + ' 秒完成上传.';
		}
	};
