...
def shop_input_func(): #获取用户选择商品的函数
    ...
    while flag:
        ...
        if quit_num == "q": #如果输入q
            ...
            while True:
                C_QUIT_NUM = input("请问您继续购物还是要退出结算? 输入Q/q结算退出,输入Y/y继续,输入C/c查询已加入到购物车到商品,输入E/e编辑购物车,输入V/v查看历史购物记录:")
...
def user_edit_gouwuche():  #编辑购物车函数
        ...
        user_edit_Fangfa = input("请选择是要添加商品还是删除商品,A/a添加,D/d为删除,C/c退出结算,W/w返回上一级:") #选择添加商品还是删除商品
        user_edit_fangfa = user_edit_Fangfa.lower()
        while True:
            if  user_edit_fangfa == "w":
                user_edit_gouwuche()
                break
            ...
            if  user_edit_fangfa == "c": #假如用户输入的是c要退出结算
                ...
                print("输入错误,请输入A/a添加商品或D/d删除商品,W/w返回上一级")
                print("您的购物车商品为%s " % list2)
                print("当前商城商品有%s" % shangpin)
                user_edit_gouwuche()
                break
